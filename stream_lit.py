...
@st.cache(allow_output_mutation=True,suppress_st_warning=True) 
def generate_chatgpt_response_v2(prompt,n, model = "gpt-3.5-turbo-16k"):
    time.sleep(n)
    responses = []
    restart_sequence = "\n"
...
def get_topics(df,n):
    ...
    for i,gm in  enumerate(gr_msg_unique):
        ...
        st.write(f"Processing request {i + 1} of {total_requests} ({progress:.2f}% complete), time:{round(elapsed_time,2)}")
        time.sleep(2)
